...
function getRatingByDishId($id) {
    ...
    // Build the condition string for the next query
    while ($row = mysqli_fetch_assoc($res)) {
    ...
    // Check if $str is empty, meaning no dish details were found
    if (empty($str)) {
        echo "<span class='rating'> (No ratings available)</span>";
        return;
    }

    $ratingLabels = array('', 'Bad', 'Below Average', 'Average', 'Good', 'Very Good');

    // Fetch the sum of ratings and the count of ratings
    ...
    // If there are ratings, calculate and display the average rating
    if ($row['total'] > 0) {
        $totalRate = $row['rating'] / $row['total'];
        echo "<span class='rating'> (" . $ratingLabels[round($totalRate)] . " rated by " . $row['total'] . " users)</span>";
    } else {
        echo "<span class='rating'> (No ratings available)</span>";
    }
}
